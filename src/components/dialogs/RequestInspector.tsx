...
export function RequestInspector({ preview, onClose }: { preview: RequestPreview; onClose: () => void }) {
  ...
  return (
    <Modal title="Request preview" onClose={onClose} wide>
      <div className="mb-4 grid grid-cols-4 gap-2 max-lg:grid-cols-3 max-sm:grid-cols-2">
        {items.map(([label, value]) => (
          <div key={label} className={`rounded-lg border px-2.5 py-2 ${label === 'Estimated input' ? 'border-border bg-muted' : 'border-border/60 bg-background'}`}>
            ...
            <strong className="mt-0.5 block text-sm font-semibold tabular-nums">{formatTokens(value)}</strong>
          </div>
        ))}
      </div>
      <div className="mb-4 grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2 max-sm:grid-cols-1">
        <span className="text-xs font-medium uppercase tracking-wide text-muted-foreground">SHA-256</span>
        <code className="min-w-0 truncate font-mono text-xs">{preview.request_sha256}</code>
      ...
      <pre className="scroll-stable max-h-[42dvh] overflow-auto whitespace-pre-wrap break-words rounded-xl border border-border/60 bg-muted/40 p-3 font-mono text-xs leading-5">{preview.request_json}</pre>
      <details className="mt-3">
        <summary className="cursor-pointer text-xs text-muted-foreground">Compiled prompt audit view</summary>
        <pre className="mt-2 scroll-stable max-h-[42dvh] overflow-auto whitespace-pre-wrap break-words rounded-xl border border-border/60 bg-muted/40 p-3 font-mono text-xs leading-5">{preview.compiled_prompt}</pre>
      </details>
    </Modal>
  )
}
