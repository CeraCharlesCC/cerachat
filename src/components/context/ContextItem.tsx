...
export function ContextItem({ slice, index, count, onChange, onDelete, onMove }: ContextItemProps) {
  ...
  return (
    <div className={`rounded-xl border border-border/60 bg-card p-2.5 transition-opacity ${slice.enabled ? '' : 'opacity-55'}`}>
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 flex-1">
          <strong className="block truncate text-[11px] font-medium" title={slice.source_name}>{slice.source_name}</strong>
          <span className="mt-0.5 block truncate text-[10px] text-muted-foreground">{range} · ~{formatTokens(slice.estimated_tokens)}</span>
        </div>
      ...
      <div className="mt-2 grid grid-cols-[minmax(0,.8fr)_minmax(0,1.2fr)_auto] gap-1.5">
        <select
          className="h-7 min-w-0 rounded-md border border-border/70 bg-background px-1.5 text-[10px] outline-none hover:bg-accent"
          value={slice.wrapper}
