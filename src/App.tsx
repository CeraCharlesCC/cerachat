  ...
  return (
    <Modal title="Provider settings" onClose={onClose} wide>
      ...
      <div className="form-grid two-column">
        ...
        <label>Protocol<select value={draft.protocol} disabled><option value="openai_chat_completions">OpenAI Chat Completions</option></select></label>
        <label className="span-2">Base URL<input value={draft.base_url} onChange={(event) => update('base_url', event.target.value)} placeholder="https://example.com/v1" /></label>
      ...
      <div className="modal-actions"><button onClick={onClose}>Cancel</button><button className="primary" disabled={saving} onClick={() => { setSaving(true); void onSave(draft).finally(() => setSaving(false)) }}>{saving ? 'Saving…' : 'Save locally'}</button></div>
    </Modal>
  )
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    void subscribeStream((payload: StreamPayload) => {
      ...
      if (payload.kind === 'started' && payload.user_message) {
  ...
  const handleSend = async () => {
    ...
    if (!text && workspaceTokens === 0) return
    setError(null)
    setBusy(true)
    try {
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar threads-panel">
        ...
        <div className="branch-tree">
          {messages.map((message) => <button key={message.id} className={`branch-node ${message.id === activeLeafId ? 'active' : ''}`} style={{ paddingLeft: `${10 + depthFor(message, byId) * 12}px` }} onClick={() => setActiveLeafId(message.id)}><span className={`role-dot ${message.role}`} /> <span>{message.content.replace(/\s+/g, ' ').slice(0, 40) || '(empty)'}</span></button>)}
        </div>
      ...
      <main className="chat-panel">
        <header className="chat-header">
          <div><span className="eyebrow">{state.provider.name}</span><strong>{state.provider.model}</strong></div>
          <div className="budget-chip"><span>{formatTokens(historyTokens + workspaceTokens + estimateTokens(textareaRef.current?.value ?? ''))} input</span><small>{formatTokens(workspaceTokens)} workspace</small></div>
        </header>
        ...
        <section className="messages">
          ...
          {path.map((message) => <article className={`message ${message.role}`} key={message.id}><div className="message-meta"><strong>{message.role === 'user' ? 'USER' : 'MODEL'}</strong><span>{shortId(message.id)}</span><label><input type="checkbox" checked={message.include_next} onChange={(event) => { const included = event.target.checked; setMessages((current) => current.map((item) => item.id === message.id ? { ...item, include_next: included } : item)); void setMessageIncluded(message.id, included).catch((reason) => setError(String(reason))) }} /> Include next</label><button title="Delete this branch" onClick={() => { if (confirm('Delete this message and all descendants?')) void deleteBranch(message.id).then(async () => { if (conversationId) await loadConversation(conversationId); await refreshBootstrap() }).catch((reason) => setError(String(reason))) }}>Delete branch</button></div><MarkdownContent content={message.content} /></article>)}
        ...
        <section className="composer-wrap">
          <div className="composer-toolbar">
            ...
            <span className="token-breakdown">{formatTokens(historyTokens)} history · {formatTokens(workspaceTokens)} workspace · ~{formatTokens(estimateTokens(textareaRef.current?.value ?? ''))} input</span>
          </div>
          <textarea ref={textareaRef} className="composer" rows={5} placeholder="Message the model…" onInput={(event) => setInputChars(event.currentTarget.value.length)} onKeyDown={(event) => { if ((event.ctrlKey || event.metaKey) && event.key === 'Enter') { event.preventDefault(); void handleSend() } }} />
          <div className="composer-footer"><span>{inputChars.toLocaleString()} chars / ~{formatTokens(Math.ceil(inputChars / 4))}</span><div><button onClick={() => void handlePreview()}>Request</button><button className="primary" disabled={busy || Boolean(streaming)} onClick={() => void handleSend()}>{streaming ? 'Streaming…' : busy ? 'Starting…' : 'Send'}</button></div></div>
        </section>
      </main>
